...
def simulate_single_sale(general_state, simulation_date, sale_timestamp, conversation_id, global_conversation_id):
    ...
    weather_msg = "It's raining" if is_raining else "☀️ Nice weather"
...
def run_full_day_simulation(num_conversations=None, simulation_date=None, lat=None, lon=None):
    ...
    general_state = init_general_state(7, lat=lat, lon=lon)

    try:
        weather = fetch_weather(lat=general_state["lat"], lon=general_state["lon"], date=simulation_date.strftime("%Y-%m-%d"))
        general_state["is_raining"] = weather.get("is_raining", False)
        print(f"Using forecast weather for {simulation_date.strftime('%Y-%m-%d')}")
    except Exception as e:
