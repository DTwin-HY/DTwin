...
# #     monkeypatch.setattr(
# #         "src.services.mcp_client.MultiServerMCPClient",
# #         lambda *a, **k: DummyClient(),
# #     )
# #     monkeypatch.setattr("langchain_openai.ChatOpenAI", lambda *a, **k: DummyLLM())
# #     monkeypatch.setattr("src.services.mcp_client.create_agent", dummy_create_agent)
...
#     with patch("src.services.mcp_client.create_mcp_agent", AsyncMock(return_value=fake_agent)):
#         test_result = await invoke_mcp_agent("weather in helsinki in 01-01-1960")
...
#     with patch("src.services.mcp_client.create_mcp_agent", AsyncMock(return_value=fake_agent)):
#         with patch(
#             "src.services.mcp_client.extract_json_from_mcp",
#             MagicMock(
...
#     with patch("src.services.mcp_client._run_in_new_loop") as run_mock:
...
#     )
#     with (
#         patch("src.services.mcp_client.create_mcp_agent", AsyncMock(return_value=fake_agent)),
#         patch("src.services.mcp_client.extract_json_from_mcp", return_value=None),
#     ):
#         res = await invoke_mcp_agent("weather in helsinki")
#     assert isinstance(res, dict)
